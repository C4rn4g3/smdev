...
public Action:Command_SetNextmap(client, args)
{
	if (args < 1)
	{
		Chat(MsgType_Error, client, " Usage: sm_setnextmap <map>");
	...
	if (!IsMapValid(map))
	{
		Chat(MsgType_Error, client, " %t", "Map was not found", map);
		return Plugin_Handled;
	}
...
public Action:Timer_StartMapVote(Handle:timer, Handle:data)
{
	...
	if (timePassed == 0 || !GetConVarBool(g_Cvar_HideTimer))
	{
		...
		switch(timerLocation)
		{
			...
			{
				ChatAll(MsgType_Info, "%t", warningPhrase, warningTimeRemaining);
			}
...
{
	ShowActivity2(client, " ", "%t", "Initiated Vote Map");
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	if (IsVoteInProgress())
	{
		...
		ChatAll(MsgType_Error, "%t", "Cannot Start Vote", FAILURE_TIMER_LENGTH);
		new Handle:data;
...
//	Call_Finish();
	...
	ChatAll(MsgType_Info, "%t", "Nextmap Voting Started");
}
						   ...
						   const item_info[][2])
{
	...
	if (num_items > 1 && GetConVarBool(g_Cvar_RunOff) && g_RunoffCount < GetConVarInt(g_Cvar_MaxRunOffs))
	{
		...
		if (highest_votes == item_info[1][VOTEINFO_ITEM_VOTES])
		{
			...
			ChatAll(MsgType_Info, "%t", "Tie Vote", GetArraySize(mapList));
		...
		else if (highest_votes < required_votes)
		{
			...
			ChatAll(MsgType_Info, "%t", "Revote Is Needed", required_percent);
			SetupWarningTimer(WarningType_Revote, MapChange:g_ChangeTime, mapList);
			return;
		}
	}
	...
	if (strcmp(map, VOTE_EXTEND, false) == 0)
	{
		...
		ChatAll(MsgType_Info, "%t", "Current Map Extended", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	{		
		ChatAll(MsgType_Info, "%t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else
	{
		...
		ChatAll(MsgType_Info, "%t", "Nextmap Voting Finished", map, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
