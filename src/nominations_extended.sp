...
public Action:Command_Addmap(client, args)
{
	if (args < 1)
	{
		Chat(MsgType_Error, client, "Usage: sm_nominate_addmap <mapname>");
	...
	{
		Chat(MsgType_Error, client, "%t", "Map was not found", mapname);
		return Plugin_Handled;		
	}
	...
	if (result > Nominate_Replaced)
	{
		...
		Chat(MsgType_Error, client, "%t", "Map Already In Vote", mapname);
	...
	Chat(MsgType_Info, client, "%t", "Map Inserted", mapname);
...
public Action:Command_Nominate(client, args)
{
	...
	if ((status & MAPSTATUS_DISABLED) == MAPSTATUS_DISABLED)
	{
		...
		{
			Chat(MsgType_Error, client, "%t", "Can't Nominate Current Map");
		}
		
		if ((status & MAPSTATUS_EXCLUDE_PREVIOUS) == MAPSTATUS_EXCLUDE_PREVIOUS)
		{
			Chat(MsgType_Error, client, "%t", "Map in Exclude List");
		}
		
		if ((status & MAPSTATUS_EXCLUDE_NOMINATED) == MAPSTATUS_EXCLUDE_NOMINATED)
		{
			Chat(MsgType_Error, client, "%t", "Map Already Nominated");
		}
	...
	ChatAll(MsgType_Info, "%t", "Map Nominated", name, mapname);
...
public Handler_MapSelectMenu(Handle:menu, MenuAction:action, param1, param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if (result == Nominate_AlreadyInVote)
			{
				Chat(MsgType_Error, param1, "%t", "Map Already Nominated");
				return 0;
			}
			else if (result == Nominate_VoteFull)
			{
				Chat(MsgType_Error, param1, "%t", "Max Nominations");
				return 0;
			}
			...
			if (result == Nominate_Replaced)
			{
				ChatAll(MsgType_Info, "%t", "Map Nomination Changed", name, map);
				return 0;	
			}
			
			ChatAll(MsgType_Info, "%t", "Map Nominated", name, map);
		}
...
stock bool:IsNominateAllowed(client)
{
	...
	switch(result)
	{
		...
		{
			Chat(MsgType_Info, client, "%t", "Nextmap Voting Started");
			return false;
		}
		
		case CanNominate_No_VoteFull:
		{
			Chat(MsgType_Error, client, "%t", "Max Nominations");
			return false;
		}
	}
