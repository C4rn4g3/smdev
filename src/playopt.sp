...
{
	name = "SM PlayOpt",
	author = "./Moriss, AlliedModders LLC",
	description = "Sound commands with option to disable it by players. Based on basic sounds plugin",
	version = "1.0",
...
{
	decl String:szBuffer[10];
	GetClientCookie(client, g_hCookie, szBuffer, sizeof(szBuffer));
	if (!StrEqual(szBuffer, "on") && !StrEqual(szBuffer, "off"))
		SetClientCookie(client, g_hCookie, "on");

	g_bCookies[client] = StrEqual(szBuffer, "on");
}
