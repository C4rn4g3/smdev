...
AttemptRTV(client)
{
	...
	{
		Chat(MsgType_Error, client, "%t", "RTV Not Allowed");
		return;
	}
		
	if (!CanMapChooserStartVote())
	{
		Chat(MsgType_Info, client, "%t", "RTV Started");
		return;
	}
	
	if (GetClientCount(true) < GetConVarInt(g_Cvar_MinPlayers))
	{
		Chat(MsgType_Error, client, "%t", "Minimal Players Not Met");
		return;			
	}
	
	if (g_Voted[client])
	{
		Chat(MsgType_Error, client, "%t", "Already Voted", g_Votes, g_VotesNeeded);
		return;
	}	
	...
	ChatAll(MsgType_Info, "%t", "RTV Requested", name, g_Votes, g_VotesNeeded);
...
StartRTV()
{
	...
	if (EndOfMapVoteEnabled() && HasEndOfMapVoteFinished())
	{
		...
		{
			ChatAll(MsgType_Info, "%t", "Changing Maps", map);
